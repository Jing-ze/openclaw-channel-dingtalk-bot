...
- **运行模式：** 多账号支持（每个账号有独立的 webhook 路径）
...
- `enabled`（可选）：默认 `true`，设为 `false` 可禁用所有账号

### 账号级配置

- `name`（可选）：账号显示名称（用于 CLI/UI 列表展示）
- `secretKey`（必填）：钉钉机器人安全密钥（通常以 `SEC` 开头）
- `webhookPath`（可选）：入站回调路径，默认 `/dingtalk-channel/{accountId}/message`
- `accessToken`（可选）：用于富文本图片下载和主动出站投递的钉钉 access token
- `enabled`（可选）：默认 `true`，设为 `false` 可禁用该账号
- `blockStreaming`（可选）：默认 `true`；设为 `true` 时，分块回复会在生成时立即发送，而非等待最终聚合
- `toolProgress`（可选）：`off` 或 `simple`，默认 `simple`；控制私聊中的工具进度提示
- `toolProgressInGroup`（可选）：`off` 或 `simple`，默认 `simple`；控制群聊中的工具进度提示

## 钉钉回调设置

在钉钉机器人配置中，为每个账号设置回调地址。每个账号有独立的 webhook 路径：

- 如果配置了 `webhookPath`：使用配置的路径
- 如果未配置 `webhookPath`：使用 `/dingtalk-channel/{accountId}/message`

例如，账号 ID 为 `sales-bot` 的回调地址：

```text
https://<your-gateway-host>/dingtalk-channel/sales-bot/message
```

使用自定义 `webhookPath` 的示例：
...
仅当配置了 `channels.dingtalk.accounts.{accountId}.accessToken` 时，主动出站才可用；未配置时该账号不支持主动出站。
...
- 主动出站依赖 `channels.dingtalk.accounts.{accountId}.accessToken`
